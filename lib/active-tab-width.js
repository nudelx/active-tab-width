...
export default {
  ...
  subscribeOnFutureScaleChange (pane) {
    if (!this.flexCache[pane.id]) {
      this.flexCache[pane.id] = pane.onDidChangeFlexScale(function (flexValue) {
        console.log('flexValue', flexValue)

      })
      pane.onWillRemoveItem(function () { console.log('destroy',arguments)})
    }
  },
  ...
  toggle() {
    ...
    allPanes.map((item) => {
      ...
      self.subscribeOnFutureScaleChange(item)
    })
  },
